...
.home-section-card {
  ...
  transition: all 0.3s ease; box-shadow: var(--shadow); position: relative; overflow: hidden;
...
(function(){
  ...
  document.querySelectorAll('a[data-spa],.nav-item,.grid-card').forEach(function(link){link.addEventListener('click',function(e){var href=this.getAttribute('href');if(!href||(!href.startsWith('/docs/')&&href!=='/'))return;if(e.metaKey||e.ctrlKey)return;e.preventDefault();var container=document.querySelector('#doc-content');container.classList.add('transitioning');setTimeout(function(){history.pushState({},'',href);fetch(href).then(function(r){return r.text();}).then(function(html){var doc=new DOMParser().parseFromString(html,'text/html');var nc=doc.querySelector('#doc-content');var ns=doc.querySelector('.sidebar');if(nc)container.innerHTML=nc.innerHTML;if(ns)document.querySelector('.sidebar').innerHTML=ns.innerHTML;document.title=doc.title;var s=href.replace('/docs/','').replace('.html','').replace('/','');if(s)markVisited(s);updateDots();window.scrollTo(0,0);requestAnimationFrame(function(){container.classList.remove('transitioning');});});},120);});});
  window.addEventListener('popstate',function(){location.reload();});

  if(navigator.geolocation){navigator.geolocation.getCurrentPosition(function(pos){var lat=pos.coords.latitude,lon=pos.coords.longitude;var r='Global';if(lat>6&&lat<38&&lon>68&&lon<98)r='India';else if(lat>24&&lat<50&&lon>-130&&lon<-60)r='North America';else if(lat>-10&&lat<55&&lon>-20&&lon<55)r='Europe';else if(lat>-10&&lat<55&&lon>95&&lon<155)r='Asia Pacific';var b=document.getElementById('location-badge');if(b){b.textContent=r;b.style.display='inline-block';}},function(){},{timeout:5000});}
})();
`;
