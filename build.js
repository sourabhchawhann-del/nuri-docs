...
function buildSidebar(activeFile) {
  return SECTIONS.map(section => {
    ...
    return `<div class="nav-section">
      <div class="nav-section-title">${section.icon} ${section.title}</div>
      ${items}
    </div>`;
  }).join('');
}
...
/* Main content */
.main {
  flex: 1; margin-left: 260px; padding: 40px 48px 80px; max-width: 860px;
}
...
.content h2 {
  font-size: 20px; font-weight: 600; margin: 32px 0 10px; color: var(--text);
...
function downloadLogo() {
  ...
  try {
    execSync(`curl -sL "https://nuri.shubhamos.com/logo.png" -o "${logoPath}"`, { timeout: 10000 });
  } catch (e) {
    console.log('  Warning: Could not download logo.png');
  }
}
